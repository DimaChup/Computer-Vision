...
class VisualFlightMission:
    ...
    def select_target_on_map(self):
        ...
        if hasattr(self, 'sim_target_px'):
            ...
            # We don't burn it in here, dynamic rendering used instead
    ...
    def get_god_view(self, cx, cy):
        ...
        if self.sim_target_px is not None:
            if self.sim_target_type == "dummy" and self.dummy_img is not None:
                # Calculate size on map
                map_h_px = int(DUMMY_HEIGHT_M * self.pix_per_m)
                # Ensure minimum visibility (e.g. 10px) so it's not invisible on large maps
                map_h_px = max(10, map_h_px) 
                
                # Overlay Dummy on Global Map
                self.overlay_image_alpha(display_map, self.dummy_img, 
                                       self.sim_target_px[0], self.sim_target_px[1], 
                                       0, map_h_px)
            else:
    ...
    def update_dashboard(self):
        ...
        # 1. Vision Logic (Run on CLEAN frame)
        found, u, v = self.eyes.process_frame_manually(drone_frame)
        
        # 2. Draw Crosshair on Drone View (Center Indicator)
