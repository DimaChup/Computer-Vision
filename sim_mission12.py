...
class VisualFlightMission:
    ...
    def overlay_image_alpha(self, background, overlay, x, y, target_w, target_h, rotation_deg=0):
        """ 
        Overlays a transparent PNG onto the background with scaling AND rotation.
        x, y: Center coordinates in background
        """
        if overlay is None: return
        
        # 1. Scale
        h_src, w_src = overlay.shape[:2]
        # Preserve aspect ratio if target_w is 0
        if target_w == 0:
        ...
        except: return # Too small to render
        
        # 2. Rotate
        # Pad image to allow rotation without cropping corners
    ...
    def select_target_on_map(self):
        ...
        if hasattr(self, 'sim_target_px'):
            ...
            # IMPORTANT: We do NOT draw it on self.full_map here anymore.
            # It will be rendered dynamically in get_drone_view
    ...
    def get_drone_view(self, cx, cy):
        ...
        # 2. Create the Background Crop (Grass)
        # Rotated crop from the big map
        ...
        # 3. UPSAMPLE to Screen Resolution (Blurry Grass)
        # This is where we create the "Canvas" for the high-res dummy
        final_view = cv2.resize(crop, (IMAGE_W, IMAGE_H))
            
        # 4. Dynamic Target Overlay (High Res on top of final_view)
        if self.sim_target_px is not None:
            # Calculate where the target is relative to the Drone Center (in map pixels)
            dx = self.sim_target_px[0] - cx
            dy = self.sim_target_px[1] - cy
            
            # Rotate vector by -yaw to align with Camera Frame
            # (Map rotates +Yaw, so points rotate -Yaw relative to center)
            angle_rad = -self.yaw
            dx_rot = dx * math.cos(angle_rad) - dy * math.sin(angle_rad)
            dy_rot = dx * math.sin(angle_rad) + dy * math.cos(angle_rad)
            
            # Convert to Screen Pixels
            # scale = Screen Width / Camera View Width (in map pixels)
            scale = IMAGE_W / max(1, self.view_w_px)
            
            # Center of screen is (IMAGE_W/2, IMAGE_H/2)
            screen_x = int((IMAGE_W / 2) + (dx_rot * scale))
            screen_y = int((IMAGE_H / 2) + (dy_rot * scale))
            
            # Calculate Dummy Size in Screen Pixels
            # Pixels per meter on SCREEN = IMAGE_W / ground_w_meters
    ...
    def get_god_view(self, cx, cy):
        ...
        if self.sim_target_px is not None:
            if self.sim_target_type == "dummy":
                cv2.circle(display_map, self.sim_target_px, 3, (0, 0, 255), -1) 
            else:
        ...
        # Resize to Dashboard Slot
        target_h = IMAGE_H
        base_scale = target_h / self.map_h
        target_w = int(self.map_w * base_scale)
        # Handle aspect ratio change due to zoom
        if self.zoom_level > 1.0:
             return cv2.resize(display_map, (IMAGE_H, IMAGE_H)) # Square-ish
