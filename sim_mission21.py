...
VERIFY_ALT = 15.0 # Low Altitude for Verification
...
class State:
    ...
    SEARCH = "SEARCH"
    CENTERING = "CENTERING"     # Align at 35m
    DESCENDING = "DESCENDING"   # Drop to 15m while refining
    VERIFY = "VERIFY"           # Check confidence at 15m
    APPROACH = "APPROACH"       # Fly to 7.5m offset
    LANDING = "LANDING"
    MANUAL = "MANUAL"
    DONE = "DONE"
...
class VisualFlightMission:
    def __init__(self):
        ...
        # Note: In new vision.py, AI is prioritized if available
        ...
        self.current_conf = 0.0 # Store detection confidence
    ...
    def update_dashboard(self):
        ...
        # Call Vision with updated signature (expecting 4 values now)
        ...
        if found:
            ...
            cv2.putText(drone_frame, f"ACQUIRED ({conf*100:.0f}%)", (10, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
    ...
    def run(self):
        ...
        while True:
            ...
            # --- UPDATED LOGIC: CENTERING -> DESCENDING -> VERIFY ---
                ...
                if self.alt <= (VERIFY_ALT + 1.0): # Within 1m of 15m
                 ...
                 if time.time() - self.last_req > 2.0:
                     if self.current_conf > 0.75:
                          ...
                          # For simulation, we print warning but proceed if it's visible, or abort?
                          # Let's assume we retry search or just hold.
                          # Here we will hold and print.
