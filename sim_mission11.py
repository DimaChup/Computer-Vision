...
class State:
    ...
    APPROACH = "APPROACH"   
    LANDING = "LANDING"
    MANUAL = "MANUAL"       # <--- New Manual State
    DONE = "DONE"
...
class VisualFlightMission:
    def __init__(self):
        ...
        # 2. Dummy Size (Scale 1.8m to pixels)
        self.dummy_h_px = int(DUMMY_HEIGHT_M * self.pix_per_m)
        self.dummy_h_px = max(5, self.dummy_h_px) 
        
        print(f"Map Scale: 1m = {self.pix_per_m:.2f} px")
        print(f"Red Dot: {self.target_radius_px} px radius")
        print(f"Dummy: {self.dummy_h_px} px tall")
    ...
    def overlay_image(self, background, overlay, x, y, target_h):
        """ Overlays a transparent PNG onto the map at specific coords """
        if overlay is None: return
        
        # Resize overlay to match target height (preserving aspect ratio)
        h, w = overlay.shape[:2]
        scale = target_h / h
        new_w = int(w * scale)
        new_h = target_h
        ...
        # Calculate bounds
        y1, y2 = y - new_h // 2, y + new_h // 2
        x1, x2 = x - new_w // 2, x + new_w // 2
        
        # Handle Clipping (if near edge of map)
        h_bg, w_bg = background.shape[:2]
        
        # Clip source/dest coordinates
        ...
        # Calculate offset in the overlay image
        ov_y1 = y1_c - y1; ov_y2 = ov_y1 + (y2_c - y1_c)
        ov_x1 = x1_c - x1; ov_x2 = ov_x1 + (x2_c - x1_c)
        
        overlay_crop = resized[ov_y1:ov_y2, ov_x1:ov_x2]
        bg_crop = background[y1_c:y2_c, x1_c:x2_c]
        
        # Alpha Blending
        if overlay_crop.shape[2] == 4: # If has alpha channel
            ...
            background[y1_c:y2_c, x1_c:x2_c] = cv2.bitwise_and(bg_crop, bg_crop, mask=cv2.bitwise_not(mask))
            background[y1_c:y2_c, x1_c:x2_c] += cv2.bitwise_and(overlay_crop, overlay_crop, mask=mask)
    ...
    def select_target_on_map(self):
        ...
        def mouse_callback(event, x, y, flags, param):
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                # VISUALIZE ON PREVIEW WINDOW
                if flags & cv2.EVENT_FLAG_CTRLKEY:
                    self.target_type = "dummy"
                    # Scale dummy height for the preview window
                    preview_h = int(self.dummy_h_px * scale_factor)
                    self.overlay_image(temp_vis, self.dummy_img, x, y, preview_h)
                else:
                    self.target_type = "dot"
        ...
        print("CLICK map. Left: Red Dot. Ctrl+Left: Dummy (6ft). Press KEY to start.")
        cv2.waitKey(0)
        cv2.destroyWindow("Select Target")
        
        if hasattr(self, 'target_px'):
            # Draw PERMANENTLY on the full resolution map
            if self.target_type == "dummy" and self.dummy_img is not None:
                self.overlay_image(self.full_map, self.dummy_img, self.target_px[0], self.target_px[1], self.dummy_h_px)
            else:
    ...
    def get_god_view(self, cx, cy):
        ...
        # 1. Draw Overlays (Drone, FOV box, Target, Landing)
        ...
        # 2. Apply Zoom (Center on Drone)
        if self.zoom_level > 1.0:
            h, w = display_map.shape[:2]
            crop_h = int(h / self.zoom_level)
            crop_w = int(w / self.zoom_level)
            
            # Center the crop on the drone (cx, cy)
        ...
        # 3. Resize to Dashboard Slot
        target_h = IMAGE_H
        # Maintain original aspect ratio for the dashboard slot width
    ...
    def on_dashboard_mouse(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_MOUSEWHEEL:
            if flags > 0: # Scroll Up
                self.zoom_level = min(self.zoom_level * 1.2, 20.0) # Max 20x zoom
            else:         # Scroll Down
                self.zoom_level = max(self.zoom_level / 1.2, 1.0)  # Min 1x zoom
    ...
    def run(self):
        ...
        # Init Window and Mouse Callback
        ...
        while True:
            ...
            if self.state == State.INIT:
                ...
                # Interpret Keyboard Inputs for Manual Flight
                vx = 0; vy = 0; vz = 0; yaw_rate = 0
                SPEED = 3.0 # m/s
                
                if key == ord('w'): vx = SPEED     # Forward
                elif key == ord('s'): vx = -SPEED  # Backward
                elif key == ord('a'): vy = -SPEED  # Left
                elif key == ord('d'): vy = SPEED   # Right
                elif key == ord('q'): yaw_rate = -0.5 # Yaw Left
                elif key == ord('e'): yaw_rate = 0.5  # Yaw Right
                elif key == 82: vz = -1.0 # Up Arrow (Climb)
                elif key == 84: vz = 1.0  # Down Arrow (Descend)

                # Send Velocity Command (Body Frame Relative)
                type_mask = 0b010111000111 # Ignore Pos & Accel, Use Vel + YawRate
                self.master.mav.set_position_target_local_ned_send(
                    0, self.master.target_system, self.master.target_component,
                    mavutil.mavlink.MAV_FRAME_BODY_OFFSET_NED,
                    type_mask,
