...
class VisualFlightMission:
    ...
    def get_god_view(self, cx, cy):
        ...
        if len(self.nfz_polygon) > 1:
             pts = np.array(self.nfz_polygon, np.int32)
             cv2.polylines(display_map, [pts], True, (0, 0, 255), 2)
             
             # Visualize Buffer (Red Glow)
             buffer_px = int(NFZ_BUFFER_M * self.pix_per_m)
             cv2.polylines(display_map, [pts], True, (0, 0, 200), buffer_px)
    ...
    def generate_search_pattern(self):
        ...
        if inset_px > 0:
            ...
            # Use erosion to shrink the search area
            kernel_size = 2 * inset_px + 1
            kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (kernel_size, kernel_size))
            eroded_mask = cv2.erode(mask, kernel)
            
            # Safety check: If erosion deletes the whole area (polygon too small), revert.
        ...
        while True:
            ...
            if key == ord('m') or key == ord('M'):
                if self.state != State.MANUAL:
                    ...
                    # SMART RESUME: If target found manually, go to CENTERING
                    if target_found:
