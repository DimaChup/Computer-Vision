...
class VisualFlightMission:
    ...
    def update_dashboard(self):
        ...
        if self.state == State.MANUAL:
            ...
            cv2.putText(drone_frame, "!!! SAFETY HALT !!!", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
            cv2.putText(drone_frame, "NFZ BREACH DETECTED", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
            cv2.putText(drone_frame, "Press 'M' to take Manual Control", (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
        
        if self.target_lat != 0:
            cv2.putText(drone_frame, f"Tgt GPS: {self.target_lat:.6f}, {self.target_lon:.6f}", (10, 210), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        if self.landing_lat != 0:
            cv2.putText(drone_frame, f"Land GPS: {self.landing_lat:.6f}, {self.landing_lon:.6f}", (10, 240), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255), 2)
        if self.state == State.DONE:
             cv2.putText(drone_frame, f"FINAL LANDING: {self.final_dist:.2f}m from Target", (10, 270), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        
        if self.state == State.SEARCH:
             cv2.putText(drone_frame, f"WP: {self.wp_index}/{len(self.waypoints)}", (10, 270), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
    ...
    def generate_search_pattern(self):
        ...
        # 5. Generate Perimeter Waypoints (Closest Edge First)
        if len(self.virtual_polygon) > 0:
            ...
            # Find point closest to drone current location
            curr_px = self.gps_to_pixels(self.lat, self.lon)
            dists = np.sum((cnt_pts - curr_px)**2, axis=1)
            closest_idx = np.argmin(dists)
            
            # Reorder points to start from closest
            ordered_pts = np.roll(cnt_pts, -closest_idx, axis=0)
