...
class VisualFlightMission:
    def __init__(self):
        ...
        self.view_w_px = 100
        self.view_h_px = 100
        self.zoom_level = 1.0 # Default Zoom (1.0 = Full Map)
    ...
    def select_target_on_map(self):
        ...
        # Helper to draw stickman with thick lines
        def draw_stickman(img, center, r):
            x, y = center
            # Thickness relative to size, ensure at least 2px
            t = max(2, int(r/2)) 
            l = r * 3 # Body length
            
            # Black Body (B,G,R) = (0,0,0)
            cv2.line(img, (x, y+r), (x, y+r+l), (0,0,0), t) # Torso
            cv2.line(img, (x-r, y+r+int(r/2)), (x+r, y+r+int(r/2)), (0,0,0), t) # Arms
            cv2.line(img, (x, y+r+l), (x-int(r/1.5), y+r+l+r), (0,0,0), t) # Left Leg
            cv2.line(img, (x, y+r+l), (x+int(r/1.5), y+r+l+r), (0,0,0), t) # Right Leg
            
            # Red Head (drawn last to be on top)
        ...
        def mouse_callback(event, x, y, flags, param):
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                # Check for Control Key (CV_EVENT_FLAG_CTRLKEY = 8) or similar depending on OS
                # Standard OpenCV flag for CTRL is usually 8 or 9 depending on platform/version
                if flags & cv2.EVENT_FLAG_CTRLKEY:
                    self.target_type = "stickman"
                    draw_stickman(temp_vis, (x, y), vis_radius)
                else:
                    self.target_type = "dot"
        ...
        print("CLICK map. Left: Red Dot. Ctrl+Left: Stickman. Press KEY to start.")
        cv2.waitKey(0)
        cv2.destroyWindow("Select Target")
        
        if hasattr(self, 'target_px'):
            # Draw permanent target on full resolution map
            if self.target_type == "stickman":
                draw_stickman(self.full_map, self.target_px, self.target_radius_px)
            else:
