...
class VisualFlightMission:
    def __init__(self):
        ...
        self.virtual_polygon = [] # The inset area
    ...
    def get_god_view(self, cx, cy):
        ...
        # Draw Virtual Area (Purple)
        if len(self.virtual_polygon) > 1:
             cv2.polylines(display_map, [self.virtual_polygon], True, (255, 0, 255), 2)
    ...
    def update_dashboard(self):
        ...
        if self.state == State.MANUAL:
            cv2.putText(drone_frame, "MANUAL CONTROL: W/S/A/D/Q/E I/K", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
        
        if self.target_lat != 0:
            cv2.putText(drone_frame, f"Tgt GPS: {self.target_lat:.6f}, {self.target_lon:.6f}", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        if self.landing_lat != 0:
            cv2.putText(drone_frame, f"Land GPS: {self.landing_lat:.6f}, {self.landing_lon:.6f}", (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255), 2)
        if self.state == State.DONE:
             cv2.putText(drone_frame, f"FINAL LANDING: {self.final_dist:.2f}m from Target", (10, 210), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        
        if self.state == State.SEARCH:
             cv2.putText(drone_frame, f"WP: {self.wp_index}/{len(self.waypoints)}", (10, 210), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
    ...
    def generate_search_pattern(self):
        ...
        # --- NEW: CALCULATE INSET & VIRTUAL POLYGON ---
        # 1. Calculate ground width at target alt
        ground_width_m = (SENSOR_WIDTH_MM * TARGET_ALT) / FOCAL_LENGTH_MM
        # 2. Inset distance = Half the camera width (so edge touches edge)
        inset_dist_m = ground_width_m / 2.0
        inset_px = int(inset_dist_m * self.pix_per_m)
        
        if inset_px > 0:
            # Erode to create "Safe Area"
            ...
            if contours:
                self.virtual_polygon = contours[0] # Save largest contour
            
            # Use eroded mask for search generation if it still exists
        ...
        # Calculate Swath for Scan Lines
        overlap = 0.2
        ...
        for scan_y in range(y + step_px//2, y + h, step_px):
            ...
            if len(pixels) > 0:
                ...
                for x_start, x_end in segments:
                    pt1_x, pt1_y = x_start, scan_y
                    pt2_x, pt2_y = x_end, scan_y
                    
                    lat_1, lon_1 = self.pixels_to_gps(pt1_x, pt1_y)
                    lat_2, lon_2 = self.pixels_to_gps(pt2_x, pt2_y)
                    
                    if direction == 1:
                        wps.extend([((lat_1, lon_1)), ((lat_2, lon_2))])
                    else:
                        wps.extend([((lat_2, lon_2)), ((lat_1, lon_1))])
    ...
    def run(self):
        ...
        while True:
            ...
            # --- DESCEND & VERIFY LOGIC ---

            elif self.state == State.CENTERING:
                if target_found:
                    self.calculate_target_gps(px_u, px_v)
                
                # Align at TARGET_ALT (35m) first
                ...
                if dist < 0.5:
                    print(f"[{self.state}] ALIGNED (Error: {dist:.2f}m). Starting Descent & Refinement.")
                ...
                if self.alt <= (VERIFY_ALT + 1.0): 
                 ...
                 if time.time() - self.last_req > 2.0:
                     if self.current_conf > 0.75:
                          ...
                          print(f"[{self.state}] Low Confidence ({self.current_conf:.2f}). Holding...")
                          if time.time() - self.last_req > 10.0:
                               print("Timeout. Aborting to Search?")
                               # self.state = State.SEARCH # Optional logic
                ...
                # Calculate Landing Offset ONCE when entering this state? 
                # Better to calculate it right before moving.
                if self.landing_lat == 0:
                     print("Calculating Landing Offset...")
                     R_EARTH = 6378137.0
                     offset_dist = 7.5
                     offset_deg_lat = (offset_dist / R_EARTH) * (180 / math.pi)
                     self.landing_lat = self.target_lat + offset_deg_lat
                ...
                if time.time() - self.last_req > 2.0:
                    self.master.mav.set_position_target_global_int_send(
                        ...
                        mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT_INT,
                        0b110111111000, 
                        int(self.landing_lat * 1e7), int(self.landing_lon * 1e7), VERIFY_ALT, # Fly at 15m
                        0, 0, 0, 0, 0, 0, 0, 0)
                    self.last_req = time.time()
